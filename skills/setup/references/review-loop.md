...
judgment stays custom because the value is in the **pinned model** (the frontmatter model even
when the session runs a lighter one) and in the judgment criteria (no generalization before the
second case · no finding without an alternative).
...
| Structural judgment | `Agent(subagent_type: "{PREFIX}-refactor-reviewer")` | pinned model / medium | target file list matching `{SOURCE_GLOB}` |
...
|---|---|---|---|
| `/refactor-review` | `{PREFIX}-refactor-reviewer` | pinned model / medium | structural judgment |
| `/review-code` | `{PREFIX}-code-reviewer` | pinned model / low | rule conformance |
...
it as `subagent_type: "{PREFIX}-refactor-reviewer"` (pinned model / medium), never
`general-purpose`.
...
```
Profiling: <s> (<min>) · <tok> · tools <n> · <model>/medium
...
Dispatch as `subagent_type: "{PREFIX}-code-reviewer"` (pinned model / low), never `general-purpose`.
...
```
Profiling: <s> (<min>) · <tok> · tools <n> · <model>/low
Scale: <n> files / <total lines> lines · <n> findings
```
